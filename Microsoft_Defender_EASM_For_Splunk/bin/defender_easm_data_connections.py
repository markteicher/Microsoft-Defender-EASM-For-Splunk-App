...
Modular Input: Data Connections

Source:
- Microsoft Defender External Attack Surface Management (EASM) Control Plane API

Responsibilities:
- Retrieve configured data connections
...
API_PATH = "/dataConnections"


class DefenderEASMDataConnections(EASMModularInput):

    def collect(self):
        self.logger.info("Starting data connection collection")
        ...
        self.logger.info(
            f"Data connection ingestion complete — {total} records"
        )


def main():
    try:
        DefenderEASMDataConnections(
            asset_name="data_connections",
            sourcetype=SOURCETYPE
        ).run()
