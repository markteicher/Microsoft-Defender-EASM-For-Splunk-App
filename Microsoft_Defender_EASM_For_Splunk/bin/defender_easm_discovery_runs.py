...
SOURCETYPE = "defender:easm:discovery_run"
API_PATH = "/discoveryRuns"


class DefenderEASMDiscoveryRuns(EASMModularInput):

    def collect(self):
        self.logger.info("Starting discovery run collection")
        ...
        params = {"$top": 200}
        if last_checkpoint:
            params["$filter"] = f"properties.startDateTime gt {last_checkpoint}"
        ...
        while next_url:
            ...
            for record in records:
                ...
                ts = record.get("properties", {}).get("startDateTime")
        ...
        self.logger.info(f"Discovery run ingestion complete — {total} records")


def main():
    try:
        DefenderEASMDiscoveryRuns(
            asset_name="discovery_runs",
